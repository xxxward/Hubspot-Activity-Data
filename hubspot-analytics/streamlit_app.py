...
st.set_page_config(page_title="HubSpot Sales Analytics", page_icon="📊", layout="wide")
...
try:
    ...
    st.info("Verify `.env`, service account JSON, and that the sheet is shared with the service account email.")
    st.stop()
...
with tab_act:
    ...
    if not scores.empty:
        st.dataframe(
            scores.style.background_gradient(subset=["activity_score"], cmap="Greens"),
...
with tab_pipe:
    ...
    if not ctq.empty:
        ...
        st.dataframe(ctq[show] if show else ctq, use_container_width=True, hide_index=True)
    else:
    ...
    with col_s:
        ...
        if not dbs.empty:
            st.dataframe(dbs, use_container_width=True, hide_index=True)

    with col_w:
        st.subheader("Win Rate")
        wr = _frep(_fpipe(data.win_rate))
        if not wr.empty:
            st.dataframe(wr.style.format({"win_rate": "{:.1%}"}),
                         use_container_width=True, hide_index=True)
...
with tab_term:
    ...
    if not wvl.empty:
        st.dataframe(wvl.style.background_gradient(subset=["net"], cmap="RdYlGn"),
                     use_container_width=True, hide_index=True)
    else:
    ...
    with col_n:
        ...
        st.dataframe(ncr, use_container_width=True, hide_index=True) if not ncr.empty else st.info("No NCR data.")

    with col_so:
        st.subheader("Sales Orders Created in NS")
        so = _frep(_fpipe(data.sales_order_created))
        st.dataframe(so, use_container_width=True, hide_index=True) if not so.empty else st.info("No data.")
    ...
    with st.expander("View All Terminal Deals"):
        ...
        if not td.empty:
            show = [c for c in ("deal_name", "hubspot_owner_name", "pipeline", "deal_stage",
                    ...
                    if c in td.columns]
            st.dataframe(td[show] if show else td, use_container_width=True, hide_index=True)
        else:
