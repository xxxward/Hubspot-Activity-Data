...
def load_all(
    ...
    raw = read_all_tabs(service_account_file, spreadsheet_id)
...
def print_summary(data: AnalyticsData) -> None:
    print("\n" + "=" * 60)
    print("HUBSPOT ANALYTICS — DATA SUMMARY")
    print("=" * 60)
    for name in ("deals", "meetings", "tasks", "tickets", "calls"):
        df = getattr(data, name)
        print(f"  {name:12s}: {len(df):>6,} rows")
    print()
    for label, df in [
        ("Rep Activity Scores", data.rep_activity_score),
        ("Active Pipeline Value", data.active_pipeline_value),
        ("Closed Won vs Lost", data.closed_won_vs_lost),
        ("Win Rate", data.win_rate),
        ("Avg Sales Cycle", data.avg_sales_cycle),
    ]:
        if not df.empty:
            print(f"── {label} ──")
